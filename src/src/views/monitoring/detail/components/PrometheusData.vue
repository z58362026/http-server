  ...
  <div class="prometheus">
    ...
    <el-row v-for="(row, rowIndex) in optionRows" :key="rowIndex" :gutter="20">
      <el-col v-for="(item, colIndex) in row" :key="colIndex" :span="12">
        <!-- 在这里放置每个选项的内容 -->
        <CustomChartLine :title="item.label" :xAxisData="months" :seriesData="salesSeries" isPreview> </CustomChartLine>
      </el-col>
    </el-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handlerSearch() {
      // TODO
      console.log('%c [  ]-72', 'font-size:13px; background:pink; color:#bf2c9f;', '搜索数据质量监控报告');
    },
  },
};
</script>
