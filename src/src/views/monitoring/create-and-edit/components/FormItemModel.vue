 ...
 * @LastEditTime: 2025-04-25 15:49:41
  ...
  <div class="form-first">
    <el-form ref="form" :model="formData" :rules="rules" label-width="180px">
      ...
      <template v-else>
        ...
        <el-row>
          <el-col :span="10" v-if="formData.dbName">
            <el-form-item label="监控数据源库名："> {{ formData.dbName }}</el-form-item>
          </el-col>
          <el-col :span="10" v-if="formData.tableName">
      ...
      <el-form-item label="推理结果表数据日期：" prop="inferenceExpress">
        <el-input
          v-model="formData.inferenceExpress"
          :maxlength="50"
          show-word-limit
          placeholder="研发系统推理结果入湖的数据日期，如：t-1m-1"
        ></el-input
      ></el-form-item>
      <el-form-item label="业务真值表数据日期：" prop="transactionExpress">
        <el-input
          v-model="formData.transactionExpress"
          :maxlength="50"
          show-word-limit
      ...
      ></el-form-item>
      <!-- 告警配置 -->
      <FormWarning ref="subFormWarning" :formData="formData" :standardOptions="standardOptions" />
    </el-form>
  </div>
</template>

<script>
import { getDataSourceFieldConfig, checkMonitoringJobName } from '@/api/monitoring-create';
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        inferenceExpress: [{ required: true, message: '请选择', trigger: 'blur' }],
        transactionExpress: [{ required: true, message: '请选择', trigger: 'blur' }],
      },
    };
  },
  computed: {
