<template>
  <div>
    ...
    <custom-table :tableData="tableData" :selection="false" :pagination="pagination" @changePage="handleChangePage">
      ...
      <el-table-column prop="modelName" label="监控模型" width="200" show-overflow-tooltip> </el-table-column>
      ...
      <el-table-column prop="createPerson" label="任务创建人" width="200" show-overflow-tooltip> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: 'monitoringCreate' });
    },
    handleEdit(item) {
      this.$router.push({ name: 'monitoringEdit', query: { id: item.id } });
    },
    handleDetail(item) {
      this.$router.push({ name: 'monitoringDetail', query: { id: item.id } });
    },
  },
};
</script>
